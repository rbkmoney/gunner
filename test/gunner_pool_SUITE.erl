...
-define(ASSERT_COUNTERS(PoolID, Free, Total), begin
    ?assertEqual(Total, get_pool_total_size(PoolID)),
    ?assertEqual(Free, get_pool_free_size(PoolID))
end).
...
acquire_free_ok_test(C) ->
    Ticket = make_ref(),
    InitialFree = get_pool_free_size(?POOL_NAME(C)),
    InitialTotal = get_pool_total_size(?POOL_NAME(C)),
    %% Initial connection creation
    ok = client_process(fun() ->
        {ok, Connection} = gunner_pool:acquire(?POOL_NAME(C), {"localhost", 8080}, Ticket, 1000),
        ?ASSERT_COUNTERS(?POOL_NAME(C), InitialFree, InitialTotal + 1),
        ok = gunner_pool:free(?POOL_NAME(C), Connection, 1000)
    end),
    ?ASSERT_COUNTERS(?POOL_NAME(C), InitialFree + 1, InitialTotal + 1).
...
connection_reuse_test(C) ->
    ...
    _ = client_process(fun() ->
        ...
        {ok, Connection2} = gunner_pool:acquire(?POOL_NAME(C), {"localhost", 8080}, Ticket, 1000),
        ?assertNotEqual(Connection1, Connection2),
        ?ASSERT_COUNTERS(?POOL_NAME(C), InitialFree - 2, InitialTotal),
        ok = gunner_pool:free(?POOL_NAME(C), Connection2, 1000),
        ?ASSERT_COUNTERS(?POOL_NAME(C), InitialFree - 1, InitialTotal),
        {ok, Connection3} = gunner_pool:acquire(?POOL_NAME(C), {"localhost", 8080}, Ticket, 1000),
        ?assertEqual(Connection3, Connection2),
        ?ASSERT_COUNTERS(?POOL_NAME(C), InitialFree - 2, InitialTotal)
    end),
    ?ASSERT_COUNTERS(?POOL_NAME(C), InitialFree, InitialTotal).
...
cant_free_multiple_times(C) ->
    ...
    ?assertEqual(InitialFree, get_pool_free_size(?POOL_NAME(C))),
    ?assertEqual(InitialTotal, get_pool_total_size(?POOL_NAME(C))).
...
cancel_acquire_test(_C) ->
    ...
    ?ASSERT_COUNTERS(PoolID, InitialFree, InitialTotal),
    _ = timer:sleep(1000),
    ?ASSERT_COUNTERS(PoolID, InitialFree + 1, InitialTotal + 1),
...
strict_connection_ownership_test(C) ->
    ...
    ?ASSERT_COUNTERS(?POOL_NAME(C), InitialFree, InitialTotal + 2),
    ?assertNotEqual(Connection1, Connection2),
    ...
    ?ASSERT_COUNTERS(?POOL_NAME(C), InitialFree + 2, InitialTotal + 2).
